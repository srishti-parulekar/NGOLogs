...
public class SearchDashboard extends javax.swing.JFrame {
    ...
    public SearchDashboard(int userId,String fname, String lname) {
        this.fname=fname;
        this.lname=lname;
        this.userId=userId;
        initComponents();
...
////                    + "WHERE person.userId = ?";
//
//            String query = "SELECT person.*, donor.amountContributed "
//                    + "FROM person "
//                    + "JOIN donor ON person.userId = donor.user_ID "
//                    + "WHERE person.userId = ?";
//
...
//            } else {
//                resultArea.setText("No contributor found with ID: " + id);
//            }
//            rs.close();
//            pstmt.close();
//            conn.close();
...
private void filter(int id) {
    ...
    try {
        ...
        String donorQuery = "SELECT COUNT(*) FROM donor WHERE user_ID = ?";
        pstmt = conn.prepareStatement(donorQuery);
        pstmt.setInt(1, id);
        rs = pstmt.executeQuery();
        rs.next();
        boolean isDonor = rs.getInt(1) > 0;
        rs.close();
        pstmt.close();

        // Check if the user is a volunteer
        String volunteerQuery = "SELECT COUNT(*) FROM volunteer WHERE user_id = ?";
        pstmt = conn.prepareStatement(volunteerQuery);
        pstmt.setInt(1, id);
        rs = pstmt.executeQuery();
        rs.next();
        boolean isVolunteer = rs.getInt(1) > 0;
        rs.close();
        pstmt.close();

        // Retrieve user information
        String userQuery = "SELECT * FROM person WHERE userId = ?";
        ...
        if (rs.next()) {
            ...
            {
                sb.append("User is both a Volunteer and a Donor !").append(" \n\n");
            }
            
            if(isVolunteer)
            {
                sb.append("User is a Volunteer !").append(" \n\n");
            }
            
            if(isDonor)
            {
                sb.append("User is a Donor !").append(" \n\n");
            }
            ...
            if (isVolunteer) {
                String volunteerInfoQuery = "SELECT * FROM volunteer WHERE user_id = ?";
                pstmt = conn.prepareStatement(volunteerInfoQuery);
                pstmt.setInt(1, id);
                rs = pstmt.executeQuery();
                if (rs.next()) {
                    String skills = rs.getString("skills");
            ...
            if (isDonor) {
                String donorInfoQuery = "SELECT * FROM donor WHERE user_ID = ?";
                pstmt = conn.prepareStatement(donorInfoQuery);
                pstmt.setInt(1, id);
                rs = pstmt.executeQuery();
                if (rs.next()) {
                    int amountContributed = rs.getInt("amountContributed");
                    sb.append("Amount Contributed: ").append(amountContributed).append(" \n\n");
                }
                rs.close();
                pstmt.close();
            }
            ...
            resultArea.setText("No user found with ID: " + id);
        }
    } catch (SQLException e) {
        e.printStackTrace();
    } finally {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        // TODO add your handling code here:
        close();
        new NGODashboard(userId,fname,lname).setVisible(true);
